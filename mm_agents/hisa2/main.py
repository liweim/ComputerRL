...
- `subgoal`: the current phase-level objective you are working on. Keep it stable across related actions and change it only when you intentionally move to a new stage.
...
class HiSA:
    ...
    def _step_abstraction(self, before_screenshot: Optional[bytes], after_screenshot: Optional[bytes],
            ...
            roi_margin: int = 50, execution_context: Optional[Dict[str, Any]] = None) -> Dict[str, str]:
